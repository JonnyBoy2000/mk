...
class Audio:
    ...
    @commands.command(name="all", hidden=True, no_pm=True)
    @checks.is_owner()
    ...
    async def pause(self, ctx):
        """Pauses the current song, r!!resume to continue."""
        server = ctx.message.server
        if not self.voice_connected(server):
            ...
            if msg.author.voice_channel == vchan:
                if self.can_instaskip(msg.author):
                    ...
                    if self._get_queue_repeat(server) is False:
                        self._set_queue_nowplaying(server, None)
                    k = discord.Embed(description="**Skipping...**", colour=discord.Colour(value=colour))
                else:
                    if msg.author.id in self.skip_votes[server.id]:
                        self.skip_votes[server.id].remove(msg.author.id)
                        reply = discord.Embed(description="**I removed your vote to skip.**", colour=discord.Colour(value=colour))
